...
Grasslands = [Pokemon("Oddish","Grass",29,29,0,False,["Razor Leaf","Tackle"],[random.randint(15,20),random.randint(10,20)]),Pokemon("Tailow","Normal",20,20,0,False,["Peck","Wing Attack"],[random.randint(15,25),random.randint(30,35)]), Pokemon("Bellsprout", "Grass",30,30,0,False, ["Acid", "Bullet Speed"], [random.randint(15, 25), random.randint(10,20)]), Pokemon("Pidgey", "Flying", 20,20,0,False,["Areal Ace", "Air Cutter"], [random.randint(40, 50), random.randint(45, 55)])]


Forest = [Pokemon("Weedle","Bug",25,25,0,False,["Poison Sting","Tackle"],[random.randint(25,35),random.randint(10,20)]), Pokemon("Wurmple","Bug",30,30,0,False,["Poison Sting","Tackle"],[random.randint(15,25), random.randint(10,20)]),Pokemon("Phantump","Ghost",35,35,0,False,["Shadow Ball","Shadow Claw"],[random.randint(20,25),random.randint(25,30)]), Pokemon("Ekans", "Poison", 30,30,0,False,["Acid","Beat Up"], [random.randint(30,40), random.randint(10,20)])]


River =[Pokemon("Electrike", "Electric", 35, 35, 0, False,["Tackle", "Shockwave"], [random.randint(10,20), random.randint(20, 30)]), Pokemon("Magikarp", "Water", 15, 15, 0, False, ["Splash", "Better Splash"], [0, 0]), Pokemon("Spheal", "Water", 35, 35, 0, False, ["Rollout", "Water Gun"], [random.randint(20, 30), random.randint(25, 35)]), Pokemon("Dratini", "Dragon",40,40,0,False,["Dragon Rage","Spark"],[random.randint(15,20),random.randint(25,30)])]


Dessert = [Pokemon("Cubone", "Ground", 20, 20, 0, False, ["Mud-Slap", "Boomerang"], [random.randint(15, 25)]), Pokemon("Onix", "Rock", 35, 35, 0, False, ["Tackle", "Sand Tomb"], [random.randint(10,20), random.randint(35,45)]), Pokemon("Gible", "Dragon", 30, 30, 0, False, ["Sand Tomb", "Tackle"], [random.randint(25, 35), random.randint(10,20)]), Pokemon("Ponyta", "Fire", 35, 35, 0, False, ["Tackle", "Tail Whip"], [random.randint(10,20), random.randint(25, 35)])]


Mountain = [Pokemon("Machop", "Fighting", 45, 45, 0, False, ["Revenge", "Low Sweep"], [random.randint(45, 50)]), Pokemon("Beldum", "Steel", 35, 35, 0, False, ["Iron Head", "Tackle"], [random.randint(45, 55), random.randint(10,20)]), Pokemon("Vanillite", "Ice", 30, 30, 0, False, ["Icy Wind", "Avalanche"], [random.randint(45, 55), random.randint(50, 60)]), Pokemon("Mankey", "Fighting", 40, 40,0,False,["Scratch", "Karate Chop"], [random.randint(30,40), random.randint(40,50)])]


Haunted_house = [Pokemon("Duskull", "Ghost", 20, 20,0,False,["Astonish", "Pursuit"], [random.randint(20, 30), random.randint(30, 40)]), Pokemon("Pinsir", "Bug", 60, 60, 0, False, ["Body Slam", "Brick Break"], [random.randint(50,60), random.randint(60,70)]), Pokemon("Ralts", "Psychic", 30,30,0,False,["Body Slam", "Double Edge"], [random.randint(50,60), random.randint(60,70)]), Pokemon("Absol", "Dark", 65,65,0,False,["Bite", "Cut"], [random.randint(50,60), random.randint(40,50)])]


def encounter(player: Player,location: Dict[str,Area]) -> None:
    if location == AREA["Grasslands"] or location == AREA["Prairie"] or location == AREA["Pampas"] or location == AREA["Savanna"]:
    ...
    if location == AREA["The Forest Opening"] or location == AREA["Temperate"] or location == AREA["Boreal"] or location == AREA["Tropical"]:
        start = random.randint(1,4)
        if start == 1:
            pass
        elif start == 2 or start == 3 or start == 4:
            ...
            print(f"The wild {wild_poke.name} appears.")
            encounter_options(player,wild_poke)
    if location == AREA["Kyoger Falls"] or location == AREA["Useless River"] or location == AREA["Great Pond"]:
        start = random.randint(1,4)
        if start == 1:
            pass
        elif start == 2 or start == 3 or start == 4:
            ...
            print(f"The wild {wild_poke.name} appears.")
            encounter_options(player,wild_poke)
    if location == AREA["Poke Beach"] or location == AREA["Dust Bowl"] or location == AREA["Addax"]:
        start = random.randint(1,4)
        if start == 1:
            pass
        elif start == 2 or start == 3 or start == 4:
            ...
            print(f"The wild {wild_poke.name} appears.")
            encounter_options(player,wild_poke)
    if location == AREA["Base"] or location == AREA["Regice Line"] or location == AREA["Peak"]:
      start = random.randint(1,4)
      if start == 1:
          pass
      elif start == 2 or start == 3 or start == 4:
          ...
          print(f"The wild {wild_poke.name} appears.")
          encounter_options(player,wild_poke)
    if location == AREA["Haunted House"] or location == AREA["Lobby"] or location == AREA["Upstairs"] or location == AREA["Attic"]:
      start = random.randint(1,4)
      if start == 1:
          pass
      elif start == 2 or start == 3 or start == 4:
...
def Giratina(player: Player,location: Dict[str,Area])-> None:
  ...
  Distortion = Pokemon("Giratina", "Ghost", 300, 300, 0, False, ["Shadow Force", "Hex"],[random.randint(60,70), random.randint(90,100)])
  ...
  while True:
        ...
        if attempts < attempt:
            ...
            if action == "Run":
                ...
                print("You have no time to be dumb, please catch it before you loose it!")
        else:
...
def catch(player: Player,location: Dict[str,Area]) -> Pokemon:
    if location == AREA["Grasslands"] or location == AREA["Prairie"] or location == AREA["Pampas"] or location == AREA["Savanna"]:
        ...
        return wild_poke
    elif location == AREA["The Forest Opening"] or location == AREA["Temperate"] or location == AREA["Boreal"] or location == AREA["Tropical"]:
        num = random.randint(0,3)
        wild_poke = Forest[num]
        return wild_poke
    elif location == AREA["Kyoger Falls"] or location == AREA["Useless River"] or location == AREA["Great Pond"]:
      num = random.randint(0,3)
      wild_poke = River[num]
      return wild_poke
    elif location == AREA["Poke Beach"] or location == AREA["Dust Bowl"] or location == AREA["Addax"]:
      num = random.randint(0,3)
      wild_poke = Dessert[num]
      return wild_poke
    elif location == AREA["Base"] or location == AREA["Regice Line"] or location == AREA["Peak"]:
      num = random.randint(0,3)
      wild_poke = Mountain[num]
      return wild_poke
    elif location == AREA["Haunted House"] or location == AREA["Lobby"] or location == AREA["Upstairs"] or location == AREA["Attic"]:
      num = random.randint(0,3)
...
def battlecounter_options(player: Player,location: Dict[str,Area]) -> None:
    if location == AREA["Battle Counter"]:
        while True:
            print("Welcome to the Battle Counter! What Tower would you like tobattle? or press q to quit.\nBronze\nSilver\nGold")
            action = input("> ")
            if action == "Bronze":
...
def bronze_tower(player: Player,location: Dict[str,Area]) -> None:
    print("This is the Bronze Tower you will have fight The Bronze Champion, Jacen's Pokemon.")
    while True:
...
def silver_tower(player: Player,location: Dict[str,Area]) -> None:
    print("This is the Silver Tower you will have fight The Silver Champion, Randy's Pokemon.")
    while True:
...
def gold_tower(player: Player,location: Dict[str,Area]) -> None:
    print("This is the Gold Tower you will have fight The Gold Champion, Dylan Shipton's Pokemon.")
    while True:
        ...
        if destination in AREA:
            if destination in location.connections:
                ...
                return location
                  ...
                  print(
                    f"You cannot go to {destination}!"
                )
        else:
...
def pick_starter(player: Player) -> None:
    while True:
        print(f"Hello, {player.name} welcome to the newly opened Safari Park, please select a starter.")
        print("Treecko, Mudkip or Torchic?")
        starter = input("> ")
        if starter == "Treecko":
            player.party.append(Pokemon("Treecko","Grass",35,35,0,False,["Pound","Absorb"],[random.randint(15,20),random.randint(25,26)]))
            print("You got the grass type Pokemon Treecko!")
            break
        elif starter == "Mudkip":
            player.party.append(Pokemon("Mudkip", "Water", 45,45,0,False,["Tackle","Water Gun"],[random.randint(10,20),random.randint(25,30)]))
            print("You got the water type Pokemon Mudkip!")
            break
        elif starter == "Torchic":
            player.party.append(Pokemon("Torchic", "Fire", 30, 30, 0, False, ["Scratch", "Ember"],[random.randint(15,20),random.randint(10, 26)]))
            print("You got the fire type Pokemon Torchic!")
            break
        else:
            player.party.append(Pokemon("Magikarp", "Water", 15, 15, 0, False, ["Splash", "Better Splash"], [0, 0]))
...
def main() -> None:
    ...
    player = Player(player_name,[],1000,["Pokeball","Pokeball","Pokeball","Pokeball","Pokeball"])
    pick_starter(player),
    while True:
      print(f"You are in the {location.name}")
      print(location.name)
